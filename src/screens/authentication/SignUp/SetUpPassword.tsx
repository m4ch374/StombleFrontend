...
const PasswordCheck: React.FC<TPasswordCheck> = ({ children, criteria }) => {
  return (
    <Text className={`${criteria() ? "text-green-500/90" : "text-[#ABABAB]"} text-[14px]`}>
      <Text>{criteria() ? "✔ " : "• "}</Text>
      <Text>{children}</Text>
    </Text>
  )
        ...
        <View className="px-4 pb-10 flex justify-between h-full">
          <View>
            <View className='mb-[24px]'>
              <Text className='text-white text-[16px] font-semibold' style={{ fontFamily: 'Lato-400' }}>
            ...
            */}
            <View className='flex pl-4 pb-10'>
              <PasswordCheck criteria={() => { return testRegexp(regexpCollection.charNum) }}>
                At least 8 characters
              </PasswordCheck>
              <PasswordCheck criteria={() => { return testRegexp(regexpCollection.hasUpper) }}>
                An uppercase character
              </PasswordCheck>
              <PasswordCheck criteria={() => { return testRegexp(regexpCollection.hasLower) }}>
                A lowercase character
              </PasswordCheck>
              <PasswordCheck criteria={() => { return testRegexp(regexpCollection.hasNum) }}>
                A number
              </PasswordCheck>
              <PasswordCheck criteria={() => { return testRegexp(regexpCollection.hasSpecChar) }}>
                A special character
              </PasswordCheck>
            </View>
            ...
            { password === confirm &&
              <Text className="text-[#ABABAB] text-[14px] text-green-500/90">✔ Password match</Text>
            }
