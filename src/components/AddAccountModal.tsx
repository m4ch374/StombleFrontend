...
const AddAccountModal: React.FC<Props> = ({ setModalVisible, navigation }) => {
  ...
  return (
    <Pressable className='w-screen h-screen fixed top-0 left-0 flex justify-end z-50' 
    ...
    >
      ...
      >
        ...
        <Pressable onPress={()=>setPress('Log into existing account')} 
          onLongPress={()=>{ navigation.navigate('LoginWithAccount'); setModalVisible(false) }}
          className='h-[48px] w-full rounded-[5px] justify-center items-center' 
          style={press === 'Log into existing account' && { backgroundColor:'#0B52BC' }}
        >
